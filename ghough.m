function [h, theta, rho] = ghough(f, dtheta, drho)
%GHOUGH  ªÙ∑Ú±‰ªª
...
% actuaπ§angle spacing will be 90 I ceil(90/DTHETA).
% 
% THETA is an NTHETA-element vector co«“taining the angle (in
% degrees) corresp°£nding to each c°£lumn of H. RHO is an
...
for k= 1:ceil(length(val)/1000)
    ...
    % constructs a sparse matrix, accu.mulates values when input
    % indices are repeated. That °Øs the behavior we want for the
    % Hough transform. We want the °£utput to be a full (non.sparse)
